...
import { TaboutHeroData, aboutHero } from "@/types";
...
export default function EditForm({ response }: any) {
	...
	} = useForm<TaboutHeroData>({
		resolver: zodResolver(aboutHero),
		defaultValues: {
	...
	const onSubmits = async (data: TaboutHeroData) => {
		try {
			await axios.patch(`/api/aboutpage/hero/${response.id}`, data);
