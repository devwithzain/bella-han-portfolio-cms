...
export async function GET(request: Request) {
	try {
		const getData = await prisma?.contactFormSection.findMany();
...
export async function POST(request: Request) {
	try {
		...
		const post = await prisma?.contactFormSection.create({
			data: {
				title,
				paragraph,
			},
...
export async function DELETE(request: Request) {
	try {
		...
		const getData = await prisma?.contactFormSection.delete({
			where: {
				id
			}
		});
