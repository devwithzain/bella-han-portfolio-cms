...
export async function GET(request: Request) {
	try {
		const getData = await prisma?.workMaterialsSection.findMany();
...
export async function POST(request: Request) {
	try {
		...
		const post = await prisma?.workMaterialsSection.create({
			data: {
				title,
				heading,
				imageUrl,
...
export async function DELETE(request: Request) {
	try {
		...
		const getData = await prisma?.workMaterialsSection.delete({
			where: {
				id
			}
		});
